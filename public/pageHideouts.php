  ...
  <nav class="m-4">
    <ul class="pagination pagination-lg justify-content-center">
      <li class="page-item">
        <?php
        if ($currentPage == 1) { ?>
          <a class="page-link disabled" href="pageHideouts.php?page=<?=$currentPage ?>">Précédent</a>
        <?php
        } else { ?>
          <a class="page-link" href="pageHideouts.php?page=<?=$currentPage - 1 ?>">Précédent</a>
        <?php
        } ?>
      </li>
      <?php
      for ($i = 1; $i <= $pageTotal; $i++) {
        if ($i != $currentPage) { ?>
          <li class="page-item"><a class="page-link" href="pageHideouts.php?page=<?=$i ?>"><?php echo $i ?></a> </li>
        <?php
        } else { ?>
          <li class="page-item active">
            <a class="page-link" href="pageHideouts.php?page=<?=$i ?>"><?php echo $i ?></a>
          </li>
      <?php
        }
      }
      ...
      <li class="page-item">
        ...
        if ($currentPage == $pageTotal) { ?>
          <a class="page-link disabled" href="pageHideouts.php?page=<?=$currentPage ?>">Suivant</a>
        <?php
        } else { ?>
          <a class="page-link" href="pageHideouts.php?page=<?=$currentPage + 1 ?>">Suivant</a>
        <?php
        } ?>
      </li>
    </ul>
